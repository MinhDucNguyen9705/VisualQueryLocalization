...
def validate(config, loader, model, epoch, output_dir, device, rank, wandb_run=None, ddp=True):
    ...
    return torch.tensor(metrics['st_iou']).mean().item(), torch.tensor(metrics['prob_accuracy']).mean().item()


def val_performance(config, preds, gts, prob_theta=0.5):
    final_output = postprocess_results(preds, prob_theta)
    st_iou = spatio_temporal_IoU(final_output, gts)
